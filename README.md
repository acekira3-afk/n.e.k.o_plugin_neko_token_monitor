...
挂件「≡」菜单首项「和猫娘一起玩」会恢复原版窗口并显示互动总结；退出后不会因刷新自动再次进入。底部「互动记录」可查看累计次数和最近 100 段历史。每次点击保存在本机，全部会话保留在插件用户数据目录 `recording/recording.json`（另有备份）。YUI 记忆失败会自动重试，界面明确标注待同步，不阻塞下一段记录。旧版已经覆盖的会话无法恢复。
